...
public class DataExtractionAction implements IActionExecution {
  ...
  public void execute() throws Exception {
    ...
    final String insertQuery = "INSERT INTO info.data_extraction_1234"+ "(origin_id,group_id, file_path, extracted_text,  created_on) " + " VALUES(?,?,?,?,now())";
    ...
    Thread.sleep(1000);
    coproProcessor.startConsumer(insertQuery, 2, 10, new DataExtractionAction.DataExtractionConsumerProcess(log, aMarker, action));
      ...
      try (Response response = httpclient.newCall(request).execute()) {
        ...
        if (response.isSuccessful()) {
          ...
          parentObj.add(DataExtractionOutputTable.builder()
                  ...
                  .groupId(Optional.ofNullable(entity.getGroupId()).map(String::valueOf).orElse(null)).build());
        }
      } catch (Exception e) {
  ...
  public static class DataExtractionOutputTable implements CoproProcessor.Entity {
    ...
    @Override
    public List<Object> getRowData() {
      return Stream.of(this.originId, this.groupId, this.filePath, this.extractedText).collect(Collectors.toList());
    }
  }
