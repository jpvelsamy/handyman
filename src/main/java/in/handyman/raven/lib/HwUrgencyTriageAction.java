...
public class HwUrgencyTriageAction implements IActionExecution {
    ...
    public void execute() throws Exception {
        try {
            ...
            final String insertQuery = "INSERT INTO hw_urgency_triage.hw_triage_transaction_" + action.getProcessId() + "(created_on, created_user_id, last_updated_on, last_updated_user_id, tenant_id, model_score, origin_id, paper_no, template_id, model_registry_id, doc_triage_state, status, stage, message)" +
                    "values(now(),?,now(),?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        public List<HwUrgencyTriageOutputTable> process(URL endpoint, HwUrgencyTriageInputTable entity) throws Exception {
            ...
            try (Response response = httpclient.newCall(request).execute()) {
                ...
                if (response.isSuccessful()) {
                    String hwTriageState = Optional.ofNullable(mapper.readTree(responseBody).get("doc_triage_state")).map(JsonNode::asText).orElseThrow();
                    ...
                    parentObj.add(HwUrgencyTriageOutputTable.builder()
                            ...
                            .build());
                    log.info(aMarker, "The Exception occurred in handwritten urgency triage");
                }
            } catch (Exception e) {
                parentObj.add(HwUrgencyTriageOutputTable.builder()
                        .createdUserId(Optional.ofNullable(entity.getCreatedUserId()).map(String::valueOf).orElse(null))
                        ...
                        .build());
                log.info(aMarker, "The Exception occurred in handwritten urgency triage", e);
    ...
    public static class HwUrgencyTriageOutputTable implements CoproProcessor.Entity {
        ...
        public List<Object> getRowData() {
            return Stream.of(this.createdUserId, this.lastUpdatedUserId, this.tenantId, this.modelScore,
                    ...
                    this.docTriageState, this.status, this.stage, this.message).collect(Collectors.toList());
        }
    }
}
