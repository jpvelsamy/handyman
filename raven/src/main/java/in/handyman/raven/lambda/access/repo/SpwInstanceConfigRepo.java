...
public interface SpwInstanceConfigRepo {
    ...
    @SqlQuery("SELECT " + COLUMNS + " FROM  " + DoaConstant.CONFIG_SCHEMA_NAME + "." + DoaConstant.SIC_TABLE_NAME + " where active=1 ; ")
    @RegisterBeanMapper(value = SpwInstanceConfig.class)
    List<SpwInstanceConfig> findAll();

    @SqlQuery("SELECT " + COLUMNS + " FROM  " + DoaConstant.CONFIG_SCHEMA_NAME + "." + DoaConstant.SIC_TABLE_NAME + " where instance= :instance  and active=1 ; ")
    @RegisterBeanMapper(value = SpwInstanceConfig.class)
    ...
    @SqlQuery("SELECT " + COLUMNS + " FROM  " + DoaConstant.CONFIG_SCHEMA_NAME + "." + DoaConstant.SIC_TABLE_NAME + " where instance= :instance  and variable= :variable  and active=1 order by version desc limit 1; ")
    @RegisterBeanMapper(value = SpwInstanceConfig.class)
    Optional<SpwInstanceConfig> findOne(@Bind("instance") final String instance, @Bind("variable") final String variable);


    @SqlQuery("SELECT " + COLUMNS + " FROM  " + DoaConstant.CONFIG_SCHEMA_NAME + "." + DoaConstant.SIC_TABLE_NAME + " where variable= :variable and active=1 ; ")
    @RegisterBeanMapper(value = SpwInstanceConfig.class)
    ...
    @SqlQuery("SELECT " + COLUMNS + " FROM  " + DoaConstant.CONFIG_SCHEMA_NAME + "." + DoaConstant.SIC_TABLE_NAME + " where name = :name and variable= :variable and config_type_id= :configTypeId and active=1 order by version desc limit 1; ")
    @RegisterBeanMapper(value = SpwInstanceConfig.class)
