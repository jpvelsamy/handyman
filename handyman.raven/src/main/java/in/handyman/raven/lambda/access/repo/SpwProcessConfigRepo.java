...
public interface SpwProcessConfigRepo {
    ...
    @SqlQuery("SELECT " + COLUMNS + " FROM  " + SpwProcessConfig.SCHEMA_NAME + "." + SpwProcessConfig.TABLE_NAME + " where active=1 ; ")
    @RegisterBeanMapper(value = SpwProcessConfig.class)
    List<SpwProcessConfig> findAll();

    @SqlQuery("SELECT " + COLUMNS + " FROM  " + SpwProcessConfig.SCHEMA_NAME + "." + SpwProcessConfig.TABLE_NAME + " where process= :process and active=1 ; ")
    @RegisterBeanMapper(value = SpwProcessConfig.class)
    List<SpwProcessConfig> findAllByProcess(@Bind("process") final String process);

    @SqlQuery("SELECT count(1)+1 FROM  " + SpwProcessConfig.SCHEMA_NAME + "." + SpwProcessConfig.TABLE_NAME + " where process= :process and variable= :variable ; ")
    @RegisterBeanMapper(value = SpwProcessConfig.class)
    Long getNextVersion(@BindBean final SpwProcessConfig spwProcessConfig);

    @SqlQuery("SELECT " + COLUMNS + " FROM  " + SpwProcessConfig.SCHEMA_NAME + "." + SpwProcessConfig.TABLE_NAME + " where process= :process and variable= :variable  and active=1 order by version desc limit 1; ")
    @RegisterBeanMapper(value = SpwProcessConfig.class)
