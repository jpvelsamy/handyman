...
public interface SpwCommonConfigRepo {
    ...
    @SqlUpdate("insert into " + SpwCommonConfig.SCHEMA_NAME + "." + SpwCommonConfig.TABLE_NAME + " ( variable, value, active, created_by, created_date, last_modified_by, last_modified_date, version) " +
    ...
    @SqlUpdate("UPDATE " + SpwCommonConfig.SCHEMA_NAME + "." + SpwCommonConfig.TABLE_NAME + " SET active=:active, created_by= :createdBy , created_date= :createdDate," +
    ...
    @SqlQuery("SELECT " + COLUMNS + " FROM  " + SpwCommonConfig.SCHEMA_NAME + "." + SpwCommonConfig.TABLE_NAME + " where active=1 ; ")
    @RegisterBeanMapper(value = SpwCommonConfig.class)
    List<SpwCommonConfig> findAll();

    @SqlQuery("SELECT count(1)+1 FROM  " + SpwCommonConfig.SCHEMA_NAME + "." + SpwCommonConfig.TABLE_NAME + " where variable= :variable ; ")
    @RegisterBeanMapper(value = SpwCommonConfig.class)
    Long getNextVersion(@BindBean final SpwCommonConfig spwCommonConfig);

    @SqlQuery("SELECT " + COLUMNS + " FROM  " + SpwCommonConfig.SCHEMA_NAME + "." + SpwCommonConfig.TABLE_NAME + " where variable= :variable  and active=1 order by version desc limit 1; ")
    @RegisterBeanMapper(value = SpwCommonConfig.class)
