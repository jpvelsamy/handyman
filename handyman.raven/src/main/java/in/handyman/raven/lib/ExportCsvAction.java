...
public class ExportCsvAction implements IActionExecution {
    ...
    private static final String query_stmt = "SELECT query FROM %s ;";

    public ExportCsvAction(final Action action, final Logger log, final Object exportCsv) {
        this.exportCsv = (ExportCsv) exportCsv;
        this.action = action;
        this.log = log;
        this.aMarker = MarkerFactory.getMarker(EXPORT_CSV);
    }

    @Override
    public void execute() throws Exception {
        var dbSrc = exportCsv.getSource();
        var tableName = exportCsv.getTablename();
        var execution_src = exportCsv.getExecution_source();
        var location = exportCsv.getLocation();
        var exec_stmt = String.format(query_stmt, tableName);
        log.info(aMarker, "Starting the execution with id {} dbSrc {} tableName {} location {} execution_src {}", action.getActionId(), dbSrc, tableName, location, execution_src);
        ...
        try (var con = hikariDataSource.getConnection()) {
            try (var stmt = con.createStatement()) {
                log.info("Executing sql statement {}", exec_stmt);
                var result = stmt.executeQuery(exec_stmt);
                while (result.next()) {
                    sql.add(result.getString(1));
                }
            } catch (SQLException ex) {
                log.error(aMarker, "Stopping execution, General Error executing sql for {} with for campaign {}", exec_stmt, ex);
                log.info(exec_stmt + ".exception", ExceptionUtil.toString(ex));
                throw new HandymanException("Process failed", ex);
            }
        }
        if (!sql.isEmpty()) {
            getSqlExecution(sql, execution_src, location);
        } else {
            log.error("SQL stmts are empty");
        }
    }

    private void getSqlExecution(List<String> sql, String mssql_src, String location) {

        for (var execStmt : sql) {
            final HikariDataSource hikariDataSource = ResourceAccess.rdbmsConn(mssql_src);
            log.info(aMarker, "Created a hikariDataSource for rdbms connection src {}", mssql_src);
            try (var con = hikariDataSource.getConnection()) {
                try (var stmt = con.createStatement()) {
