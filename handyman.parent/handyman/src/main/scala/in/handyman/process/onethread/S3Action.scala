...
class S3Action extends in.handyman.command.Action with LazyLogging {
  ...
  def execute(context: Context, action: in.handyman.dsl.Action, actionId: Integer): Context = {
    ...
    val name = s3.getName
    val ddlSql: String = /*s3.getValue.replaceAll("\"", "")*/"select filepath,filesize from 266_dropbox;"
    ...
    try {
      ...
      breakable
      {
          while (rs.next()) 
          {
                ...
                println("Amazon S3 Client started ")
                val tm = new TransferManager(amazonS3Client);
                logger.info("Transfer Manager started ")
                logger.info("File to be uploaded :"+directory.getName)
                resp=amazonS3Client.putObject(new PutObjectRequest(S3Bucket, directory.getName, directory))
                var S3fileSize = amazonS3Client.getObjectMetadata(S3Bucket, directory.getName).getContentLength().toString()
